...
function createRoom(start, length, width, height, createBase) {
    basement = [
...
function createRightDoorRoom(start, length, width, height, createBase) {
    basement = [
        vec3(start[0], 0.0, start[2] + length),
        vec3(start[0] + width, 0.0, start[2] + length),
        vec3(start[0], 0.0, start[2]),
...
function createLeftRooms(floor) {
    if (floor == 3 || floor == 4) {
        createRoom(vec4(min_x, 0, -450), 2 * 50, 100, 150, false);
        for (var i = 2; i < 8; i++) {
            createRoom(vec4(min_x, 0, (-450 + (i * 50))), 50, 100, 150, false);
        }
        createRoom(vec4(min_x, 0, (-450 + (8 * 50))), 2 * 50, 100, 150, false);
        for (var i = 10; i < 17; i++) {
            createRoom(vec4(min_x, 0, (-450 + (i * 50))), 50, 100, 150, false);
        }
        createRoom(vec4(min_x, 0, (-450 + (17 * 50))), 50, 100, 150, false);
    }
    createRoom(vec4(min_x, 0, (-450 + (17 * 50))), 50, 100, 150, false);
}
...
function createRightRooms(floor) {
    if (floor == 3 || floor == 4) {
        createRoom(vec4(250, 0, -450), 2 * 50, -100, 150, false);
        for (var i = 2; i < 17; i++) {
            createRoom(vec4(250, 0, (-450 + (i * 50))), 50, -100, 150, false);
        }
        createRoom(vec4(250, 0, (-450 + (17 * 50))), 50, -100, 150, false);
    }
}
...
function createLab(floor) {
    if (floor == 4) {
        // First three labs
        createRightDoorRoom(vec4(-150, 0, -600), 150, 125, 150, true);
        createRightDoorRoom(vec4(-25, 0, -600), 150, 125, 150, true);
        createRightDoorRoom(vec4(100, 0, -600), 150, 50, 150, true);

        // 2nd from last two labs
        createRoom(vec4(0, 0, 125), 75, -100, 150, false);
        createRoom(vec4(0, 0, 125), 75, 100, 150, false);

        // Last two labs
        createRoom(vec4(0, 0, 200), 200, -100, 150, false);
        createRoom(vec4(0, 0, 200), 200, 100, 150, false);

        // 2nd from first three labs
        createRoom(vec4(0, 0, -400), 200, -100, 150, false);
        createRoom(vec4(0, 0, -400), 100, 100, 150, false);
        createRoom(vec4(0, 0, -300), 100, 100, 150, false);
        // Stairs
        //  createRoom(vec4(-25, 0, -35), 70, -75, 150);
        //  createRoom(vec4(-25, 0, -35), 70, 125, 150);
        createRightDoorRoom(vec4(25, 0, -75), 50, 60, 150, false);
        createRightDoorRoom(vec4(25, 0, -25), -50, -110, 150, false);
    }
    if (floor == 3) {
        // First three labs
        createRightDoorRoom(vec4(-150, 0, -600), 150, 125, 150, true);
        ...
        // Last labs
        createRoom(vec4(-100, 0, 200), 200, 200, 150, false);

        // 2nd from first three labs
        createRoom(vec4(0, 0, -400), 200, -100, 150, false);
        createRoom(vec4(0, 0, -400), 200, 100, 150, false);
        //createRoom(vec4(0, 0, -300), 100, 100, 150);
        // Stairs
        //  createRoom(vec4(-25, 0, -35), 70, -75, 150);
        //  createRoom(vec4(-25, 0, -35), 70, 125, 150);
        createRightDoorRoom(vec4(25, 0, -75), 50, 60, 150, false);
...
function createRestroom(floor) {
    if (floor == 3 || floor == 4) {
        createRoom(vec4(-100, 0, -200), 50, 50, 150, false);
        createRoom(vec4(100, 0, -200), 50, -50, 150, false);
    }
}
...
function createElevator(floor) {
    ...
    if (floor == 3 || floor == 4) {
        //createRightDoorRoom(vec4(100, 0, 125), -40, -200, 150);
    ...
    if (currentlyPressedKeys[70]) { // F, allow floor plan view
        ...
        if (viewfloortimer0 > 10) {
            if (!viewfloorTop) {
                ...
                up = vec3(0.0, 1.0, 0.0);
                eye = vec3(-600, 1000, 0); // Initial at  elevator
                at = vec3(0, 10, 0);
                personheight = 1200;
                directionMat = mat3(
                    [-1, 0, 0],
